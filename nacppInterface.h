...
public:
    ...
    */
    ...
    /**
    @brief Освобождает динамическую память, выделенную для передачи результата от сервиса.
    @detailed Большинство функций интерфейса возвращают char *, динамически выделенную строку.
    Для корректного освобождения памяти, учитывая разныю компиляторы и пр, рекомендуется вызывать
    эту функцию для предотвращения утечек памяти.
    @param char * mem
    */
